...
files. Every behavior proven by a hermetic test corpus (43 units /
545 assertions) that runs on your machine, offline, before anything ships.

```
./install.sh                 # pinned toolchain, hermetic
bash scripts/test.sh         # 43u/545a — prove the install
...
| **Monitoring auto-generated from the registry** | gatus + dashboard re-render on every deploy; every service's health page exists because the registry says so. |
...
| Hermetic test corpus proving itself | ✅ 43u/545a | ❌ | ❌ | ❌ | ❌ |
