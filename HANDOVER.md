...
| Cluster | k3s v1.36.3+k3s1 single node (namespace **`fleet`** — the sos-lab namespace is DELETED); `/etc/rancher/k3s/registries.yaml` HTTP mirror + `/etc/hosts` ClusterIP entry for `docker-registry.fleet.svc.cluster.local`; `/workspace` → symlink → `/home/openchamber/workspaces` (recreated 2026-08-30 post-move; the old `/root/workspaces` target is gone) |
